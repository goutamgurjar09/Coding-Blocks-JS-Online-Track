...
//Now The event loop is a constantly running process that monitors/traking both the callback queue and the callstack.
...
//if callstack empty hai to ye callback queue se function ko callstack me pass/place kr dega and callstack  function ko execute kr dega
...
// If the call stack is not empty ,the event loop waits until it is empty
// as soon as callstack is empty the event loop moves functions from the callback queue to the call stack for execution.
...
// Here's a breakdown how does work  event loop :
...
//--------------------
//Call stack / Execution stack:- Execution stack , also known as calling stack  ->
//ita a stack with a LIFo(Last in First Out) which is used to store all the execution context created during the code execution
...
// JavaScript is a single-threaded language, means it has only one execution thread  means it will run/perform one task at a time, sequentially.
